// YourWill quality pass v2
// Branch: cozy-current only. Visible gameplay, worker animations, clean HUD, better coast/selection.
(function(){
  const fx=[];
  window.ywFx=fx;
  let selected=null;
  let qualityMsg='Quality pass v2 loaded';

  const style=document.createElement('style');
  style.textContent=`#yw-info-panel{display:none!important}.card{opacity:.96}`;
  ...
  function state(){return window.ywState||(window.ywState={wood:0,food:0,stone:0,mode:'hand',speed:1})}

  // Stronger readable coast while the exact external sand pack is selected later.
  ...
  drawTile=function(q){
    ...
    if(q.type==='shore'){
      ctx.save();
      ctx.globalAlpha=.88;pathD(p.x,p.y+2,TW*.96,TH*.52);ctx.fillStyle='#d8b166';ctx.fill();
      ctx.globalAlpha=.40;pathD(p.x,p.y-1,TW*.66,TH*.30);ctx.fillStyle='#f3d98b';ctx.fill();
      ctx.globalAlpha=.22;ctx.strokeStyle='#8b6838';ctx.beginPath();ctx.moveTo(p.x-22,p.y+4);ctx.lineTo(p.x-7,p.y+8);ctx.moveTo(p.x+4,p.y+1);ctx.lineTo(p.x+19,p.y+5);ctx.stroke();
      ctx.restore();
    }else if(q.type==='water'){
      ctx.save();ctx.globalAlpha=.16;pathD(p.x,p.y-2,TW*.62,TH*.28);ctx.fillStyle='#bdeaf0';ctx.fill();ctx.restore();
    }else if((q.type==='grass'||q.type==='flower'||q.type==='bushTile')&&((q.x*13+q.y*19)%6===0)){
      ctx.save();ctx.globalAlpha=.10;pathD(p.x+3,p.y+1,TW*.34,TH*.16);ctx.fillStyle='#a0cc61';ctx.fill();ctx.restore();
    }
  };
  ...
  updatePeople=function(dt){
    ...
    for(const ch of chars){
      ...
      if(!busy && (!ch.wait||ch.wait<10)){
        const bp=nearest(ch,structures,o=>o.kind==='blueprint'&&!o.built&&st.wood>=8&&st.stone>=3);
      ...
      if(ch.jobObj&&['chop','mine','forage','build'].includes(ch.need)){
        ...
        if((flora.includes(o)||structures.includes(o))&&Math.hypot(ch.x-o.x,ch.y-o.y)<.20){
          ...
          if(ch.work>1.0){
            ch.work=0;
            if(ch.need==='build'){
              if(st.wood>=8&&st.stone>=3){st.wood-=8;st.stone-=3;o.kind='house';o.built=true;o.s=.62;addFx(o.x,o.y,'text','Дом');qualityMsg='Дом построен';setCard('Дом построен','Жители построили дом.');sortStatic()}
            }else{
              const r=resOf(o);if(r){st[r]=(st[r]||0)+1;addFx(o.x,o.y,'text','+'+r);qualityMsg='+'+r}
            ...
            ch.need='idle';ch.jobObj=null;ch.wait=16;
          }
        }
      }
    }
  ...
  person=function(ch){
    ...
    const p=iso(ch.x,ch.y,0),s=.49*(ch.body||1),swing=Math.sin((ch.work||0)*22);
    ctx.save();ctx.translate(p.x,p.y-5+Math.abs(swing)*1.2);ctx.scale(s*ch.dir,s);
    ...
    ctx.save();ctx.translate(7,-10);ctx.rotate(-.9+swing*.75);ctx.fillStyle='#6b4325';ctx.fillRect(0,-13,4,27);ctx.fillStyle=ch.need==='mine'?'#8f8f84':ch.need==='build'?'#d5b15f':'#c78b35';ctx.fillRect(-3,-17,10,6);ctx.restore();
  ...
  drawLayer=function(list){
    for(const o of list){
      if(o.kind==='blueprint'){
        const p=iso(o.x,o.y,0);ctx.save();ctx.translate(p.x,p.y);ctx.scale(o.s||.72,o.s||.72);ctx.globalAlpha=.88;dia(0,14,58,29,'#d1b16a','rgba(255,245,180,.6)');ctx.strokeStyle='#ffe08a';ctx.lineWidth=3;ctx.strokeRect(-19,-8,38,27);ctx.beginPath();ctx.moveTo(-25,-8);ctx.lineTo(0,-30);ctx.lineTo(25,-8);ctx.stroke();ctx.restore();continue;
      }
      if(o.kind==='house'){
  ...
  selectAt=function(clientX,clientY){oldSelect(clientX,clientY);selected=null;const sx=clientX*DPR,sy=clientY*DPR;function sp(o,z=0){const p=iso(o.x,o.y,z),ox=cam.x/(cam.z*DPR)-40,oy=cam.y/(cam.z*DPR)-18;return{x:(p.x+ox)*cam.z*DPR,y:(p.y+oy)*cam.z*DPR}}let bd=999;for(const o of [...chars,...structures,...flora]){const p=sp(o,0),d=Math.hypot(p.x-sx,p.y-sy);if(d<55*DPR&&d<bd){bd=d;selected=o}}};
  ...
  draw=function(){
    ...
    for(const ch of chars){if(['chop','mine','forage','build'].includes(ch.need)){const p=iso(ch.x,ch.y,26),label=ch.need==='chop'?'🪓':ch.need==='mine'?'⛏':ch.need==='forage'?'🍓':'🔨';ctx.font='16px sans-serif';ctx.fillText(label,p.x-8,p.y-10)}}
    ...
    ctx.restore();
    const st=state();ctx.save();ctx.setTransform(1,0,0,1,0,0);const y=72*DPR,x=42*DPR,w=Math.min(canvas.width-84*DPR,560*DPR),h=34*DPR;ctx.fillStyle='rgba(8,24,20,.88)';ctx.fillRect(x,y,w,h);ctx.strokeStyle='rgba(240,220,150,.24)';ctx.strokeRect(x,y,w,h);ctx.fillStyle='#f6e4aa';ctx.font=(12*DPR)+'px system-ui,sans-serif';ctx.fillText(`День 1 · 👥 ${chars.length} · 🍓 ${Math.floor(st.food||0)} · 🪵 ${Math.floor(st.wood||0)} · 🪨 ${Math.floor(st.stone||0)} · ${qualityMsg}`,x+12*DPR,y+22*DPR);ctx.restore();
  };
  qualityMsg='анимации работы включены';setCard('Quality v2','Рубка, добыча, стройка, FX и HUD включены.');
})();
