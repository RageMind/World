...
(function(){
  ...
  function s(){return window.ywState||(window.ywState={wood:0,food:0,stone:0,mode:'hand',speed:1})}
  ...
  updatePeople=function(dt){
    ...
    for(const ch of chars){
      ...
      if(!busy && (!ch.wait||ch.wait<8)){
        ...
        const pick=(tree&&rock)?(dd(ch,tree)<dd(ch,rock)?tree:rock):(tree||rock);
      ...
      if(ch.jobObj&&['chop','mine','forage','build'].includes(ch.need)){
        ...
        if((flora.includes(o)||structures.includes(o))&&Math.hypot(ch.x-o.x,ch.y-o.y)<.20){
          ...
          if(ch.work>1.0){
            ch.work=0;
            if(ch.need==='build'){
              if(st.wood>=8&&st.stone>=3){st.wood-=8;st.stone-=3;o.kind='house';o.built=true;o.s=.60;addFx(o.x,o.y,'text','Дом');qualityMsg='дом построен';setCard('Дом построен','Жители построили дом.');sortStatic()}
            }else{
  ...
  draw=function(){
    ...
    ctx.restore();
    const state=s();ctx.save();ctx.setTransform(1,0,0,1,0,0);
