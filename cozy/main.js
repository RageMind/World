...
function init() {
  ...
  for (let y = 0; y < mapH; y++) for (let x = 0; x < mapW; x++) {
    const q = getTile(x, y); if (!q || q.type === 'water') continue;
    if (q.type !== 'shore') {
      ...
      // V5: big visible grass detail props. These do not depend on image textures.
      const detailCount = hash(x, y, 70) < .8 ? 2 + Math.floor(hash(x, y, 71) * 3) : 1;
      for (let i = 0; i < detailCount; i++) {
        const k = hash(x, y, 80 + i);
        decor.push({
          kind: k < .20 ? 'dirt' : k < .52 ? 'grassTuft' : k < .74 ? 'leafPatch' : 'flowerDots',
          x: x + rnd(.18, .82, x, y, 90 + i),
          y: y + rnd(.18, .82, x, y, 100 + i),
          s: rnd(.45, .95, x, y, 110 + i),
          depth: x + y + .62 + i * .01
        });
      }
    }
  ...
  for (let i = 0; i < 8; i++) {
    const x = 9 + (i % 4) * 1.15 + rnd(-.22, .22, i, 1), y = 9.4 + Math.floor(i / 4) * 1.15 + rnd(-.18, .2, i, 2);
    chars.push({ x, y, c: cols[i % cols.length], p: hash(i, 2, 1) * 6.28, depth: x + y + 1.15, name: ['Alina', 'Mira', 'Torn', 'Nora', 'Kira', 'Luka'][i % 6] });
  }
}

function diamondPath(cx, cy, w, h) { ctx.beginPath(); ctx.moveTo(cx, cy - h / 2); ctx.lineTo(cx + w / 2, cy); ctx.lineTo(cx, cy + h / 2); ctx.lineTo(cx - w / 2, cy); ctx.closePath(); }
function diamond(cx, cy, w, h, fill, stroke) { diamondPath(cx, cy, w, h); ctx.fillStyle = fill; ctx.fill(); if (stroke) { ctx.strokeStyle = stroke; ctx.stroke(); } }
function drawTile(tile) {
  const p = iso(tile.x, tile.y, tile.h), x = p.x, y = p.y;
  const c = { grass: '#5d8d3b', flower: '#639746', bushTile: '#567f35', shore: '#caa15b', water: '#246b82' }[tile.type];
  if (tile.type === 'water') {
    const wave = Math.sin(t * .02 + tile.x * .7 + tile.y * .4) * .7;
    diamond(x, y + wave, TILE_W, TILE_H, '#1e6077', 'rgba(42,109,126,.55)');
    ctx.globalAlpha = .13; diamond(x, y + wave - 3, TILE_W * .68, TILE_H * .46, '#72bac2'); ctx.globalAlpha = 1;
    return;
  }
  const outline = tile.type === 'shore' ? 'rgba(82,58,30,.24)' : 'rgba(30,52,31,.13)';
  diamond(x, y, TILE_W, TILE_H, c, outline);
  if (tile.type === 'shore') {
    ctx.fillStyle = 'rgba(255,228,151,.18)';
    for (let i = 0; i < 3; i++) ctx.fillRect(x - 20 + i * 14 + tile.n * 5, y - 4 + i * 3, 7, 2);
  } else {
    ctx.fillStyle = 'rgba(232,230,130,.10)';
    for (let i = 0; i < 5; i++) ctx.fillRect(x - 26 + i * 12 + tile.n * 5, y - 7 + i * 3, 5, 2);
  }
  if (tile.type !== 'water' && tile.type !== 'shore' && (isWater(tile.x + 1, tile.y) || isWater(tile.x - 1, tile.y) || isWater(tile.x, tile.y + 1) || isWater(tile.x, tile.y - 1))) {
    ctx.globalAlpha = .16; diamond(x, y, TILE_W * .86, TILE_H * .66, '#b99152'); ctx.globalAlpha = 1;
  }
}
...
function pathPatch(x, y, s = 1) { ctx.save(); ctx.translate(x, y); ctx.scale(s, s); ctx.globalAlpha = .38; diamond(0, 0, TILE_W * .72, TILE_H * .52, '#b48343'); ctx.globalAlpha = 1; ctx.restore(); }

function grassTuft(x, y, s = 1) {
  ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
  ctx.fillStyle = 'rgba(0,0,0,.10)'; blob(0, 6, 12, 4);
  const cols = ['#79a64d', '#9fbd55', '#4f8038'];
  for (let i = -3; i <= 3; i++) {
    ctx.strokeStyle = cols[(i + 3) % cols.length]; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(i * 3, 5); ctx.lineTo(i * 4, -8 - Math.abs(i)); ctx.stroke();
  }
  ctx.restore();
}
function leafPatch(x, y, s = 1) {
  ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
  ctx.fillStyle = 'rgba(0,0,0,.08)'; blob(0, 4, 14, 5);
  ['#6fa447', '#85b858', '#4f8036'].forEach((c, i) => { ctx.fillStyle = c; blob(Math.cos(i * 2.1) * 7, Math.sin(i * 2.1) * 3, 7, 4); });
  ctx.restore();
}
function flowerDots(x, y, s = 1) {
  ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
  ctx.fillStyle = '#7fa94b'; blob(0, 4, 12, 5);
  ctx.fillStyle = '#ffcf65'; ctx.fillRect(-8, -4, 4, 4); ctx.fillRect(4, -3, 4, 4);
  ctx.fillStyle = '#f06b59'; ctx.fillRect(-1, 1, 4, 4);
  ctx.restore();
}
function dirtPatch(x, y, s = 1) {
  ctx.save(); ctx.translate(x, y); ctx.scale(s, s);
  ctx.globalAlpha = .34; ctx.fillStyle = '#a7763b'; blob(0, 0, 18, 7); ctx.globalAlpha = 1;
  ctx.fillStyle = 'rgba(255,225,150,.25)'; ctx.fillRect(-8, -2, 6, 2); ctx.fillRect(2, 1, 7, 2);
  ctx.restore();
}
...
function draw() {
  resize(); t++;
  const grd = ctx.createLinearGradient(0, 0, 0, H); grd.addColorStop(0, '#0e282a'); grd.addColorStop(.42, '#203d31'); grd.addColorStop(1, '#071614');
  ctx.fillStyle = grd; ctx.fillRect(0, 0, W, H);
  ctx.save(); ctx.scale(DPR * cam.zoom, DPR * cam.zoom); ctx.translate(cam.x / (cam.zoom * DPR) - 40, cam.y / (cam.zoom * DPR) - 18);
  tiles.sort((a, b) => (a.x + a.y) - (b.x + b.y)).forEach(drawTile);
  const items = []; decor.forEach(d => items.push(d)); chars.forEach(c => items.push({ kind: 'person', ref: c, depth: c.depth }));
  items.sort((a, b) => a.depth - b.depth).forEach(o => {
    const p = o.ref ? null : iso(o.x, o.y, 0);
    if (o.kind === 'tree') tree(p.x, p.y, o.s); else if (o.kind === 'rock') rock(p.x, p.y, o.s); else if (o.kind === 'pebble') pebble(p.x, p.y, o.s); else if (o.kind === 'bush') bush(p.x, p.y, o.s); else if (o.kind === 'grassTuft') grassTuft(p.x, p.y, o.s); else if (o.kind === 'leafPatch') leafPatch(p.x, p.y, o.s); else if (o.kind === 'flowerDots') flowerDots(p.x, p.y, o.s); else if (o.kind === 'dirt') dirtPatch(p.x, p.y, o.s); else if (o.kind === 'house') house(p.x, p.y, o.s); else if (o.kind === 'camp') campfire(p.x, p.y, o.s); else if (o.kind === 'path') pathPatch(p.x, p.y, o.s); else if (o.kind === 'person') person(o.ref);
  });
  ctx.restore(); ctx.globalAlpha = .07; ctx.fillStyle = '#000'; ctx.fillRect(0, 0, W, H); ctx.globalAlpha = 1;
  requestAnimationFrame(draw);
}
canvas.addEventListener('pointerdown', e => { cam.drag = true; cam.locked = true; cam.lx = e.clientX; cam.ly = e.clientY; });
canvas.addEventListener('pointermove', e => { if (!cam.drag) return; cam.x += e.clientX - cam.lx; cam.y += e.clientY - cam.ly; cam.lx = e.clientX; cam.ly = e.clientY; });
canvas.addEventListener('pointerup', () => cam.drag = false); canvas.addEventListener('pointercancel', () => cam.drag = false);
canvas.addEventListener('wheel', e => { e.preventDefault(); cam.zoom = Math.max(.45, Math.min(1.8, cam.zoom * (e.deltaY > 0 ? .9 : 1.1))); }, { passive: false });
init(); draw();
