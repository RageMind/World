...
function draw(){resize();const now=performance.now(),dt=Math.min(.05,(now-last)/1000);last=now;t++;updatePeople(dt);const g=ctx.createLinearGradient(0,0,0,H);g.addColorStop(0,'#0e282a');g.addColorStop(.42,'#203d31');g.addColorStop(1,'#071614');ctx.fillStyle=g;ctx.fillRect(0,0,W,H);ctx.save();ctx.scale(DPR*cam.z,DPR*cam.z);ctx.translate(cam.x/(cam.z*DPR)-40,cam.y/(cam.z*DPR)-18);for(const q of tileDraw)drawTile(q);drawLayer(groundDraw);drawLayer(pathsDraw);drawLayer(structuresDraw);drawLayer(floraDraw);chars.slice().sort((a,b)=>a.depth-b.depth).forEach(person);ctx.restore();if(art.loaded){ctx.fillStyle='rgba(20,40,25,.65)';ctx.fillRect(10*DPR,(innerHeight-28)*DPR,150*DPR,18*DPR);ctx.fillStyle='#d9ff9b';ctx.font=(10*DPR)+'px sans-serif';ctx.fillText('ANIM V1',16*DPR,(innerHeight-15)*DPR)}ctx.globalAlpha=.035;ctx.fillStyle='#000';ctx.fillRect(0,0,W,H);ctx.globalAlpha=1;requestAnimationFrame(draw)}
